...
from production.forms import DateForm

def dailyProductionReport(request):
    form = DateForm
    ...
    for i in range (0,report_hr_local+1): #plus one so as to also include data from current hour
        ...
        for category in categories:
            data = DataLog.objects.filter(created__icontains = str(db_datetime.date()) ).filter(created__icontains = str(db_datetime.date()) + " " + str(hour_prefix[0:2]), category=category)
    ...
    return render(request, 'production/daily-production-report.html',{'details':loading_details, 'dailyWeightTotal':daily_weight_total, 'date':report_date_local, 'form':form})